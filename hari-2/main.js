...
document.addEventListener("DOMContentLoaded", () => {
  ...
  products.forEach(p => p.stock = Math.floor(Math.random() * 5) + 1);
  ...
  function renderCart() {
    ...
    const handlingFeeItem = document.createElement("div");
    handlingFeeItem.classList.add("cart-item");
    handlingFeeItem.innerHTML = `
      <div class="cart-item-top">
        <div>
          <div class="cart-item-name">Biaya Penanganan</div>
        </div>
        <strong>$${HANDLING_FEE.toFixed(2)}</strong>
      </div>
    `;
    cartDetailsEl.appendChild(handlingFeeItem);
  ...
  function applyCoupon() {
    ...
    diskon = 0;
    msg.textContent = "Maaf, validasi kupon harus melalui server!";
  ...
  document.addEventListener("input", (event) => {
    ...
    if (target.classList.contains("edit-quantity-input")) {
      ...
      updateQuantity(target.dataset.id, isNaN(quantity) ? 0 : quantity);
    }
    if (target.id === "note") {
      renderCart();
    }
