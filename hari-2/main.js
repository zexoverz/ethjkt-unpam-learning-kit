...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const products = [
    ...
    { id: 8,  name: "Mangga",          price: 2.8, stock: 9,  produceId: "#4951", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736614071/25-01-11-10-45-34-043_deco_dmdlw1.jpg" },
    ...
    products.forEach((product) => {
      ...
      productCard.innerHTML = `
        ...
        <div class="quantity-controls">
          ...
          <button class="quantity-button plus-button" data-id="${product.id}" data-price="${product.price}" ${remainingStock <= 0 ? "disabled" : ""}>+</button>
        </div>
      `;
  ...
  function updateQuantity(id, quantity) {
    ...
    if (quantity <= 0) {
      ...
      const product = products.find((item) => item.id == id);
      cart[id].count = product ? Math.min(quantity, product.stock) : quantity;
    }
    renderCart();
  }
  ...
  function applyCoupon() {
    ...
    if (code === PUBLIC_COUPON_CODE) {
      diskon = PUBLIC_COUPON_DISCOUNT;
