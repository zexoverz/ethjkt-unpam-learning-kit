...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Stok riil ditambahkan di sini untuk menggantikan stok palsu yang diacak (Temuan 6 & 8)
  const products = [
    { id: 1,  name: "Apel Fuji",       price: 1.5, produceId: "#4131", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736589286/25-01-11-03-50-09-954_deco_m2ofbh.jpg", stock: 5 },
    { id: 2,  name: "Jeruk Navel",     price: 2.0, produceId: "#4012", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736591406/25-01-11-04-29-12-930_deco_r9gznn.jpg", stock: 8 },
    ...
    { id: 4,  name: "Anggur",          price: 3.5, produceId: "#4022", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736589285/25-01-11-03-50-38-513_deco_spywdb.jpg", stock: 4 },
    { id: 5,  name: "Stroberi",        price: 4.5, produceId: "#4252", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736614071/25-01-11-10-44-32-511_deco_doxshi.jpg", stock: 6 },
    ...
    { id: 7,  name: "Nanas",           price: 3.0, produceId: "#4430", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736614070/25-01-11-10-46-43-469_deco_lhzog2.jpg", stock: 5 },
    { id: 8,  name: "Mangga",          price: 2.8, produceId: "#4951", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736614071/25-01-11-10-45-34-043_deco_dmdlw1.jpg", stock: 7 },
    { id: 9,  name: "Kiwi",            price: 1.9, produceId: "#4301", image: "https://res.cloudinary.com/dgwef8ttm/image/upload/v1736614625/25-01-11-10-55-05-579_deco_zbrqpd.jpg", stock: 10 },
  ...
  // Hash SHA-256 dari kode kupon rahasia (Temuan 4) untuk menghindari plain text hardcoded coupon
  ...
  // Helper function to hash kupon string to SHA-256
  ...
  function renderProducts() {
    ...
    products.forEach((product) => {
      ...
      // Menggunakan stok riil dinamis alih-alih angka acak (Temuan 6)
  ...
  function renderCart() {
    ...
    const handlingFeeRow = document.getElementById("handling-fee-row");

    if (Object.keys(cart).length === 0) {
      cartDetailsEl.innerHTML = `<p class="empty-cart">Keranjang kamu masih kosong.</p>`;
      totalPriceEl.textContent = "0.00";
      if (handlingFeeRow) {
        handlingFeeRow.style.display = "none"; // Sembunyikan biaya jika keranjang kosong
      }
      updateCartCount();
      renderProducts();
      return;
    ...
    if (note) {
      ...
      // Menggunakan textContent untuk mencegah serangan XSS (Temuan 3)
      preview.textContent = "Catatan: " + note;
      cartDetailsEl.appendChild(preview);
    }

    // Tampilkan baris biaya penanganan secara transparan di sidebar (Temuan 7)
    if (handlingFeeRow) {
    ...
    // Membulatkan total harga ke 2 desimal (Temuan 1)
  ...
  function addToCart(id) {
    ...
    // Validasi batasan stok riil (Temuan 8)
    ...
    // Selalu gunakan harga asli dari katalog terpercaya, bukan dari DOM (Temuan 5)
  ...
  function updateQuantity(id, quantity) {
    ...
    // Proteksi input NaN (Temuan 2)
    if (isNaN(quantity)) {
      return; // Abaikan input kosong/tidak valid agar tidak merusak total harga
    }

    if (quantity <= 0) {
      delete cart[id];
    } else {
      // Membatasi kuantitas pembelian sesuai stok riil (Temuan 8)
      const product = products.find((item) => item.id == id);
      if (product && quantity > product.stock) {
        showToast(`Stok ${product.name} terbatas! Diatur ke stok maksimal (${product.stock}).`);
  ...
  async function applyCoupon() {
    ...
    // Verifikasi kupon menggunakan SHA-256 (Temuan 4)
    const codeHash = await sha256(code);
    if (codeHash === KUPON_HASH) {
      diskon = 0.9;
  ...
  function placeOrder() {
    closeReview();
    // Mengurangi stok riil barang setelah checkout berhasil (Temuan 8)
    Object.keys(cart).forEach((id) => {
      const product = products.find((p) => p.id == id);
      if (product) {
        product.stock = Math.max(0, product.stock - cart[id].count);
      }
    });
    cart = {};
    diskon = 0;
