...
// & B ke pool, dapet "shares" (bukti kepemilikan). Fee LP bisa diubah
// owner; bagian protocol fee dipisah dan dapat ditarik owner.
//
// ALUR PAKAI (di Remix):
...
//
// (Ini versi belajar. Di produksi masih perlu SafeERC20, reentrancy
// guard, dan slippage protection.)
// ============================================================
...
contract SimpleAMM is Ownable {
    ...
    // Fee menggunakan basis points (bps): 10_000 bps = 100%.
    uint256 public constant FEE_DEN = 10_000;
    uint256 public constant PROTOCOL_FEE_BPS = 5; // 0,05% dari input swap.
    uint256 public constant MAX_SWAP_FEE_BPS = 1_000; // Fee LP maksimum 10%.

    // Fee LP awal 0,30%. Owner dapat mengubahnya melalui setSwapFee().
    uint256 public swapFeeBps = 30;

    // Fee protocol tidak menjadi reserve pool. Dicatat terpisah per token
    // supaya hanya owner yang dapat menariknya lewat withdrawProtocolFee().
    ...
    event SwapFeeUpdated(uint256 previousFeeBps, uint256 newFeeBps);
    event ProtocolFeeWithdrawn(address indexed owner, uint256 amountA, uint256 amountB);

    constructor(address _tokenA, address _tokenB) Ownable(msg.sender) {
        require(_tokenA != address(0) && _tokenB != address(0), "token nol");
        require(_tokenA != _tokenB, "token sama");
        tokenA = IERC20(_tokenA);
        tokenB = IERC20(_tokenB);
    }

    // Mengubah fee yang menjadi milik liquidity provider. Contoh: 30 = 0,30%.
    // Protocol fee 0,05% tetap ditambahkan pada total fee swap.
    function setSwapFee(uint256 newSwapFeeBps) external onlyOwner {
        require(newSwapFeeBps <= MAX_SWAP_FEE_BPS, "fee terlalu besar");
        emit SwapFeeUpdated(swapFeeBps, newSwapFeeBps);
        swapFeeBps = newSwapFeeBps;
    }

    // Mengirim seluruh protocol fee token A dan B ke owner.
    // Reserve pool tidak dikurangi karena protocol fee memang tidak pernah
    // dimasukkan ke reserve saat swap.
    function withdrawProtocolFee() external onlyOwner returns (uint256 amountA, uint256 amountB) {
        amountA = protocolFeesA;
        amountB = protocolFeesB;

        protocolFeesA = 0;
        protocolFeesB = 0;

        if (amountA > 0) tokenA.transfer(owner(), amountA);
        if (amountB > 0) tokenB.transfer(owner(), amountB);

        emit ProtocolFeeWithdrawn(owner(), amountA, amountB);
    }
    ...
    // Rumus x*y=k lengkap dengan fee LP dinamis + protocol fee. Ini fungsi "view" -> kamu
    ...
    function getAmountOut(
        ...
        uint256 totalFeeBps = swapFeeBps + PROTOCOL_FEE_BPS;
        uint256 amountInWithFee = amountIn * (FEE_DEN - totalFeeBps);
    ...
    function swapAforB(uint256 amountIn) external returns (uint256 amountOut) {
        ...
        uint256 protocolFee = (amountIn * PROTOCOL_FEE_BPS) / FEE_DEN;

        tokenA.transferFrom(msg.sender, address(this), amountIn);
        tokenB.transfer(msg.sender, amountOut);

        // Fee LP tetap berada di reserve; protocol fee dipisahkan dari reserve.
        reserveA += amountIn - protocolFee;
        reserveB -= amountOut;
